...
disp(2+ysize*rows+rows-1);disp(2+xsize*cols+cols-1);
I = bgval*ones(floor(2+ysize*rows+rows-1),floor(2+xsize*cols+cols-1));
  ...
  for j=0:cols-1
    ...
    if i*cols+j+1>size(W,2)
      % This leaves it at background color
        ...
        %disp(ysize); disp(xsize);
        disp(size(W(:,i*cols+j+1)));
        %ysize = floor(ysize); xsize = floor(xsize)+1;
      % This sets the patch
